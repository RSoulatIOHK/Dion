...
def fetchBlock (sock : Socket) (point : Point) : IO (Except SocketError (Option ByteArray)) := do
  ...
  | .error e => do
      IO.println s!"✗ BlockFetch send error: {e}"
      return .error e
  | .ok () => do
      -- Receive StartBatch or NoBlocks
      match ← receiveBlockFetch sock ⟨#[]⟩ 65535 with
      | .error e => do
          IO.println s!"✗ BlockFetch receive error: {e}"
          return .error e
      | .ok none => do
          IO.println "✗ BlockFetch receive returned none (timeout or decode failure)"
          return .ok none
      | .ok (some result) =>
          match result.message with
          | .MsgStartBatch => do
              IO.println "✓ Got MsgStartBatch, receiving block..."
              -- Receive block
              ...
              | .ok (some blockResult) =>
                  ...
                  | .MsgBlock blockBytes => do
                      ...
                      | .ok (some doneResult) =>
                          ...
                          | _ => do
                              IO.println "⚠ Expected BatchDone, got different message"
                              return .ok (some blockBytes)  -- Return block anyway
                      | _ => return .ok (some blockBytes)  -- Return block anyway
                  | _ => do
                      IO.println "⚠ Expected MsgBlock"
                      return .ok none
          | .MsgNoBlocks => do
              IO.println "✗ Server has no blocks in range"
              return .ok none
          | _ => do
              IO.println "⚠ Expected MsgStartBatch or MsgNoBlocks"
              return .ok none
